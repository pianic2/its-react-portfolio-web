...
The project view model provides rendering-oriented fields such as the stable project number, primary claim status and label, repository URL, localized detail path, and deterministic visual variant. Components do not import raw content records or infer visual identity from array position.

## Components

- `ProjectShowcase` owns the semantic section and the Home or Projects composition.
- `ProjectShowcaseCard` renders project copy, capability chips, claim status, and explicit detail and repository actions.
...
- `ProjectDetailPage` uses the context for slug resolution and presents a project hero, editorial scope panels, a claim/evidence rail, and project navigation.
